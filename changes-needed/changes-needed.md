...
Status: Löst
Jag lade resurs- och turneringsvalideringen i `store.create_moderator_token()` så den gäller oavsett om moderatorn skapas via API eller intern kod. Funktionen kontrollerar nu att turneringen finns, att angiven resurs finns och att resursens `tournament_id` matchar moderatorns turnering. API-rutten fångar `ValueError` och returnerar `400`, vilket ersätter både främmande scopes och saknade resurser med kontrollerade klientfel. Regressionstestet `backend/tests/test_api.py::test_moderator_resource_scope_must_belong_to_tournament` verifierar främmande resurs, saknad resurs och att all-scope moderator utan `resource_id` fortfarande fungerar.
...
Status: Löst
Jag lade en backendspärr i `services.update_match_result()` som stoppar resultatändring om matchens status redan är `completed`. Admin-UI:t räknar nu avslutade matcher som icke-poängrapporterbara, visar `Låst` i åtgärdskolumnen och inaktiverar även avslutsknappen om en gammal dialog ändå skulle ligga kvar. Det gör att både API och UI behandlar avslutade matcher som låsta i stället för att låta bracketpropagering köras om. Regressionerna är `backend/tests/test_services.py::TournamentServiceTests::test_completed_match_result_cannot_be_changed` och den uppdaterade Docker/Chromium-testen `admin kan klicka igenom huvudflödet, liveuppdatera och avsluta match`, som verifierar låst state efter avslut.
